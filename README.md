...
### Exposed ports
- `5173:80` for frontend

说明：
- Compose 默认不直接暴露后端 `8000` 到宿主机
- 前端通过容器内服务名 `backend` 反代 API
...
当前部署模型把前端 Nginx 作为统一入口，浏览器访问 `5173` 即可，API 通过 `/api` 反代到后端容器。
